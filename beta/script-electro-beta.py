...
def analyze_ion_distributions(testno,traj):
	...
	#for pair in pairs:
	#	print pair
	#	mset.batch_gr_lipid_ion([pair[0],pair[1]],framecount=framecount,skip=skip,label=pair[2],mode='voronoi_bin',monolayer_rep='P')
	#---Save the data
	#pickledump(mset,'pkl.electro.'+tests[testno]+'.'+basename+'.pkl')
	return mset
