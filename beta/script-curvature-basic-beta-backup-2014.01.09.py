	...
	for ad in analysis_descriptors[analysis_plan]: 
		...
		if nprots > 0:
			...
			protpts = array([[i[0]*numgridpts/vecs[0],i[1]*numgridpts/vecs[1]] 
				for i in np.mean(mset.protein,axis=0)])
		...
		if nprots > 0:
			...
			protpts = array([[i[0]*numgridpts/vecs[0],i[1]*numgridpts/vecs[1]] 
				for i in np.mean(mset.protein,axis=0)])
			hull = scipy.spatial.ConvexHull(protpts)
			ax2.plot(protpts[hull.vertices,0],protpts[hull.vertices,1],'r-',lw=1)
			ax2.plot(protpts[hull.vertices,0][-1],protpts[hull.vertices,1][0],'r-',lw=1)
		cax = inset_axes(ax2,
		     width="5%",
		     height="100%",
