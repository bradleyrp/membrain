...
params = [[c,d] for c in [50,80,100,110,120,130,150,200,250] for d in [0]]
decay_z0 = True
...
if 'compute' in routine:
	...
	#---append to msdats for each new parameter set
	for param in params:
		cutoff,zfiltdir = param
		#---NEEDS test to prevent redoing the same calculation here
		for aname in analysis_names:
			for i in analysis_descriptors[aname]: vars()[i] = (analysis_descriptors[aname])[i]
			anum = analysis_names.index(aname)
			filespec = specname_guess(sysname,trajsel)
