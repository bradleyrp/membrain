...
if int(seq[2]):
	...
	center = [12,14]
	uqmean = array(abs(mean(hqs,axis=0)))
	qmags = array([[sqrt(((i-center[0])/((Lx)/1.)*2*pi)**2+((j-center[1])/((Ly)/1.)*2*pi)**2) 
		for j in range(0,n)] for i in range(0,m)])
	spectrum1d = array([[uqmean[i,j],qmags[i,j]] for j in range(n) for i in range(m)])
	#plt.scatter(spectrum1d[:,0],spectrum1d[:,1]);plt.show()
#---plot comparison
if int(seq[3]):
	islognorm = True
