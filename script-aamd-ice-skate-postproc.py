...
allrawcurves = array([mean(distsxyz[i],axis=0) for i in range(timelimit)]).T
times = [mean([clock[i+d]-clock[i] for i in range(nframes-d)]) for d in range(timelimit)]
fast = []
slow = []
...
ax.plot(array(times)/1000.,mean(fast,axis=0),c=clrs[0], lw=4, alpha=1, label='Mean of fast ions')
ax.plot(array(times)/1000.,mean(slow,axis=0),c=clrs[2], lw=4, alpha=1, label='Mean of slow ions')
...
[bz,az] = numpy.polyfit(log10(times[1:]), log10(mean(fast,axis=0)[1:]),1)
print bz, az
fitted_y = [(bz*i+az) for i in times[1:]]
ax.plot(array(times[1:])/1000.,fitted_y, c = clrs[1], lw=2, alpha = 1, label='Linear fit, slope = %.2f' %bz)

#[bz,az] = numpy.polyfit(log10(times[1:]), log10(mean(slow,axis=0)[1:]),1)
#print bz, az
#fitted_y = [(bz*i+az) for i in times[1:]]
#ax.plot(array(times[1:])/1000.,fitted_y, c = clrs[3], lw=2, alpha = 1, label='Linear fit, slope = %.2f' %bz)

fitfunc = lambda p, x: p[0] + p[1] * x
errfunc = lambda p, x, y: (y - fitfunc(p, x))
powerlaw = lambda x, amp, index: amp * (x**index)
pinit = [100.0, -100.0]
#power_fit = optimize.leastsq(errfunc, pinit, args=(log10(times[1:]), log10(mean(allrawcurves[:][1:],axis=0)[1:])), full_output=1)
power_fit = optimize.leastsq(errfunc, pinit, args=(log10(times[1:]), log10(mean(slow,axis=0)[1:])), full_output=1)
pfinal = power_fit[0]
covar = power_fit[1]
index = pfinal[1]
amp = 10.0**pfinal[0]
...
#plt.show()
plt.savefig(pickles+'fig-'+sysname.split('-')[-1]+'-allrawcurves-analysis.png',dpi=300)
plt.clf()
plt.close()
